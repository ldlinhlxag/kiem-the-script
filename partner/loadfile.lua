...
function Partner:LoadLevelSetting()
	...
	for _, tbData in pairs(tbFile) do
		...
		if not nLevel then
			print("Partner:LoadLevelSetting(), \\setting\\partner\\levelsetting.txt[Level]Tải Thất bại!");
			return;
		end
...
function Partner:LoadStarLevel()
	...
	for _, tbData in pairs(tbFile) do
		...
		if not nStarLevel then
			print("Partner:LoadStarLevel(), \\setting\\partner\\starlevel.txt[StarLevel]Tải Thất bại!");
			return;
		end
...
function Partner:LoadTalentLevel()
	...
	for _, tbData in pairs(tbFile) do
		...
		if not nTalent then
			print("Partner:LoadTalentLevel(), \\setting\\partner\\talentlevel.txt[Talent]Tải Thất bại!");
			return;
		end
...
-- tbPartnerAttrib = 
-- {
--    [nNpcTemplateId] = {[nId],[nType],[nTypeLevel],[nLifeRatio], [nRate]},
-- }
-- tbPartIdToNpcId = {[nId] = nNpcId}
function Partner:LoadPartnerAttrib()
	...
	for _, tbData in pairs(tbFile) do
		...
		if not nTemplateId then
			print("Partner:LoadPartnerAttrib(), \\setting\\partner\\partner.txt[NPCId]Tải Thất bại!");
			return;
		end
...
function Partner:LoadBaseValue()
	...
	for _, tbData in pairs(tbFile) do
		...
		if not nType then
			print("Partner:LoadBaseValue(), \\setting\\partner\\basevalue.txt[Type]Tải Thất bại!");
			return;
		end
...
function Partner:LoadMaintenance()
	...
	for _, tbData in pairs(tbFile) do
		...
		if not nSkillUpRate then
			print("Partner:LoadMaintenance(), \\setting\\partner\\maintenance.txt[SkillUpRate]Tải Thất bại!");
			return;
		end
...
function Partner:LoadSkillRule()
	...
	for _, tbData in pairs(tbFile) do
		local nType = tonumber(tbData.Type);
		if not nType then
			print("Partner:LoadSkillRule(), \\setting\\partner\\skillrule.txt[Type]Tải Thất bại!");
			return;
		end
...
function Partner:LoadSkillSetting()
	...
	for _, tbData in pairs(tbFile) do	
		...
		if not nRandomSkillTemplateId then
			print("Partner:LoadSkillSetting(), \\setting\\partner\\skillsetting.txt[RandomSkillTemplateId]Tải Thất bại!");
			return;
		end
...
function Partner:LoadPotentialTemp()
	...
	for _, tbData in pairs(tbFile) do
		...
		if not nPotenTemplateId then
			print("Partner:LoadPotentialTemp(), \\setting\\partner\\potential.txt[PotenTemplId]Tải Thất bại!");
			return;
		end
