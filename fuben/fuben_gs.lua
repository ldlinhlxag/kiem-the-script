...
function CFuben:ApplyFuBenEx(nType, nId, nPlayerId)
	...
	if pPlayer  then
		...
		if self.FubenData[nPlayerId] then
			pPlayer.Msg(string.format("您已经申请了<color=yellow> %s <color>副本，不能在申请了！",self.FUBEN[self.FubenData[nPlayerId][4]][self.FubenData[nPlayerId][5]].szName)) ;
			return 0;
		end
		...
		if self.FUBEN[nType][nId].nGroupModel == 1 then
			...
			if #tbPlayerList < self.FUBEN[nType][nId].nMinNumber or #tbPlayerList > self.FUBEN[nType][nId].nMaxNumber then
				KTeam.Msg2Team(pPlayer.nTeamId, string.format("你们队成员少于<color=yellow>%s<color>，或是多于<color=yellow>%s<color>",self.FUBEN[nType][nId].nMinNumber,self.FUBEN[nType][nId].nMaxNumber));
				return 0;
			end
			for i = 1,#tbPlayerList do
				...
				if pPlayerEx then
					if pPlayerEx.nLevel < self.FUBEN[nType][nId].nGrade or pPlayerEx.nFaction == 0 then
						KTeam.Msg2Team(pPlayerEx.nTeamId, string.format("你们队的<color=white>  %s  <color>怎么还是菜鸟，他是不能去的！",pPlayerEx.szName));
						return 0;
					end
...
function CFuben:IsSatisfy(nPlayerId, nCaptainId)	--进入副本条件	
	...
	if pPlayer then		
		...
		if self.FUBEN[nType][nId].nGroupModel == 1 then		--组队模式
			local tbPlayerList = KTeam.GetTeamMemberList(pPlayer.nTeamId);
			if #tbPlayerList < self.FUBEN[nType][nId].nMinNumber or #tbPlayerList > self.FUBEN[nType][nId].nMaxNumber then
				pPlayer.Msg(string.format("你们队成员少于<color=yellow>%s<color>，或是多于<color=yellow>%s<color>",self.FUBEN[nType][nId].nMinNumber,self.FUBEN[nType][nId].nMaxNumber));
				return 0;
			end
