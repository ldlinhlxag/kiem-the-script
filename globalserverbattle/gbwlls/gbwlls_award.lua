...
function GbWlls:OnGetAward(nFlag)
	...
	if (nMaxRank >= 10000) then
		...
		szRank = string.format("获得了<color=yellow>%s<color>", szRank);
	end
	
	if not nFlag then
		local szMsg = string.format("跨服武林联赛官员：您的战队在上届武林联赛中%s，确定现在领取吗？", szRank);
		local tbOpt = 
		{
	...
	local szFriend = string.format("您的好友[<color=green>%s<color>]在上届武林联赛中获得了<color=red>%s<color>。", me.szName, szRank);
