...
function tbNpc:OnGetStarServerAward(nSureFlag)
	...
	if (not nSureFlag or nSureFlag ~= 1) then
		Dialog:Say(string.format("恭喜恭喜！普天同庆！贵服务器在第%s届跨服联赛中总积分排名大区<color=yellow>第%s名<color>。可以获得明星服务器奖励。确定现在领取吗？", nGblSession, nStarFlag),
				{
					{"确定", self.OnGetStarServerAward, self, 1},	
					{"我来看看"},
				}
			);
		return 0;
...
function tbNpc:OnGetGuess8RankAward(nSureFlag)
	...
	if (not nSureFlag or nSureFlag ~= 1) then
		...
		if (nAwardFlag == 1) then
			...
			table.insert(tbOpt, {"确定", self.OnGetGuess8RankAward, self, 1});
		end
		table.insert(tbOpt, {"我来看看"});
	...
	if (me.CountFreeBagCell() < nBagCount) then
		Dialog:Say(string.format("Hành Trang không đủ chỗ trống%s格", nBagCount));
		return 0;
	end
...
{
	[1] = [[
    2010年4月7日-27日，循环赛期间，江湖威望排名前5000的玩家可去本服临安府跨服联赛官员旁，敲响跨服联赛助威鼓，为参赛者加油，每天一次，每次可获得一张幸运卡。
...
function tbNpc:AboutInfo(nIndex)
	Dialog:Say(string.format(self.tbAbout[nIndex]), {{"返回上层", self.About, self},{"Kết thúc đối thoại"}});
end
