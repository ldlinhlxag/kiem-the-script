...
-- 增加转出数据
function tbVipTransfer:AddApplyOut_GS(szPlayerName, tbInfo)
	GCExcute({"VipPlayer.VipTransfer:AddApplyOut_GC", szPlayerName, tbInfo});
end
...
function tbVipTransfer:CheckQualification(pPlayer)
	...
	-- 申请资格
	if pPlayer.GetTask(self.TASK_GROUP_ID, self.TASK_QUALIFICATION) == 1 then
		return 1;
	end
...
function tbVipTransfer:CheckGetAward(pPlayer)
	...
	if nGateId ~= self.tbGlobalBuffer.tbApplyIn[pPlayer.szAccount].nNewGateId then
		return 0;
	end
...
function tbVipTransfer:CalculateValue(pPlayer)
	...
	local nJbPrice = math.max(100, KJbExchange.GetPrvAvgPrice()) * 100;
	...
	tbValue.nBindValue = tbValue.nBindValue + math.floor(nPartnerValue / 20000);
	
	-- 内部返还账号检测
	if self:CheckSepcailAccount(pPlayer) ~= 1 then
		tbValue.nBindValue = math.floor(tbValue.nBindValue * 0.4);
	end
