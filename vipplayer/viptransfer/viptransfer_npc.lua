...
function tbNpc:ApplyTransfer()
	...
	if me.GetTask(VipPlayer.VipTransfer.TASK_GROUP_ID, VipPlayer.VipTransfer.TASK_QUALIFICATION) ~= 1 then
		Dialog:Say("你没有申请转服的资格。");
		return 0;
	end
	...
	for nGateId, tbInfo in pairs(VipPlayer.VipTransfer.tbServerName) do
		tbOpt[#tbOpt + 1] = {string.format("<color=green>%s<color>", tbInfo[0]), self.OnSelectGate, self, nGateId};
	end
...
function tbNpc:OnSelectGate(nGateId)
	...
	local szMsg = string.format("您打算转往<color=yellow>[%s]<color>下哪个服务器？", VipPlayer.VipTransfer.tbServerName[nGateId][0]);
	local tbOpt = {};
	
	for nId, szServerName in pairs(VipPlayer.VipTransfer.tbServerName[nGateId]) do
		if nId ~= 0 then
			tbOpt[#tbOpt + 1] = {string.format("<color=green>%s<color>", szServerName), self.OnSelectServer, self, nGateId, nId};
		end
	end
	
	tbOpt[#tbOpt + 1] = {"返回上层", self.ApplyTransfer, self};
	Dialog:Say(szMsg, tbOpt);	
end

-- 选择服务器后
function tbNpc:OnSelectServer(nGateId, nId)
	Dialog:AskString("请输入通行证：", 10, self.OnEnterAccount, self, nGateId, nId);
end

-- 输入通行证后
function tbNpc:OnEnterAccount(nGateId, nId, szAccount)
	
	local szMsg = string.format("您确定转往区服<color=yellow>[%s-%s]<color>，转入通行证：<color=yellow>%s<color>", 
		VipPlayer.VipTransfer.tbServerName[nGateId][0], VipPlayer.VipTransfer.tbServerName[nGateId][nId], szAccount);
...
不绑定价值量：<color=green>%s<color>
	...
	local tbOpt =
	{
		{"确定", self.OnConfirm, self, szAccount, nId},
		{"我再想想"},
	};
	Dialog:Say(szMsg, tbOpt);
end
...
-- 确定转服
function tbNpc:OnConfirm(szAccount, nId)
	...
	-- 增加数据项
	VipPlayer.VipTransfer:AddApplyOut_GS(me.szName, tbInfo);
	...
	Dbg:WriteLog("VipTransfer", "Vip转服申请", me.szAccount, me.szName, "绑定价值量："..tbInfo.nBindValue, "不绑定价值量："..tbInfo.nNoBindValue,
		"转向账号："..tbInfo.szNewAccount, "转向网关："..tbInfo.nNewGateId, "保留声望数量："..#tbInfo.tbRepute);
...
function tbNpc:OnGetRemainRepute(nIndex)
	...
	if nOpenTime >= nDay * 60 * 60 * 24  then
		...
		me.Msg(string.format("您的<color=yellow>%s<color>已经提升至<color=yellow>%s级<color>", VipPlayer.VipTransfer.tbReputeName[nIndex][1], nLevel));
	elseif nTaskId == 9 and nLevel >= 2 and me.GetTask(VipPlayer.VipTransfer.TASK_GROUP_ID, 7) == 0 then
		Dialog:Say("武林大会声望暂时不开放领取，您可以选择兑换为秦陵·发丘门声望。", {{"<color=yellow>我要兑换<color>.", self.ChangeHSB, self, nTaskId, nLevel}, {"我再想想"}});
	else
		Dialog:Say(string.format("服务器开服时间不满%s天，暂时无法领取该项声望", nDay));
	end
end

function tbNpc:ChangeHSB(nTaskId, nLevel)
	local tbRepute = 
	{
		[2] = 5000,
		[3] = 15000,
	};
	if not tbRepute[nLevel] then
		return 0;
	end
	me.SetTask(VipPlayer.VipTransfer.TASK_GROUP_ID, nTaskId, 0);
	Player:AddRepute(me, 9, 2, tbRepute[nLevel]);
end
	...
	if TimeFrame:GetState("OpenLevel150") == 1 then
		...
		for i = 1, #VipPlayer.VipTransfer.tbTimeLevel do
			...
			if tbLevel and nOpenTime >= tbLevel[1] * 60 * 60 * 24 then
				me.AddLevel(tbLevel[2] - me.nLevel);
				return 0;
			end
		end
