...
function tbXiJiu:CanUse(pItem)
	...
	if (0 == Marry:CheckWeddingMap(me.nMapId)) then
		szErrMsg = "Bạn không có trong buổi lễ, bạn không thể sử dụng";
		return 0, szErrMsg;
	end
	...
	if (0 == bIsCurMapItem) then
		szErrMsg = "Hai món quà không phù hợp bạn không thể sử dụng";
		return 0, szErrMsg;
	end	
...
function tbShouPengHua:CanUse(pItem)
	local szErrMsg = "";
	if (0 == Marry:CheckWeddingMap(me.nMapId)) then
		szErrMsg = "Bạn không có trong buổi lễ bạn không thể sử dụng vật phẩm";
		return 0, szErrMsg;
	end
