...
--===================================================
tbNpc.MAXNUM_LIST		= 10;		-- 最多列出前20名送礼金的来宾
...
function tbNpc:ShowGift()
	...
	local szMsg = string.format("您当前送给二位侠侣的礼金一共是%s两。以下是送出礼金总额排名靠前的宾客列表。\n\n", nMyselfCost);
	for _, tbInfo in ipairs(tbRecordList) do
		szMsg = szMsg .. string.format("<color=yellow>%s<color>送来礼金<color=yellow>%s<color>两\n",
			tbInfo.szName, tbInfo.nSum);
	end
	Dialog:Say(szMsg);
end
