...
function tbMission:StartGame(szMaleName, szFemaleName, nDynMapId, nMapLevel, nWeddingLevel, nStartTime, nEndTime)
	...
	Dbg:WriteLog("Marry", "Hệ Thống Kết Hôn", 
		string.format("%s và %s tổ chức đám cưới", szMaleName, szFemaleName),
		string.format("Tổ chức ngày: %s", os.date("Giờ: %H Ngày: %d Tháng: %m Năm: %Y", GetTime())),
		string.format("Đẳng cấp điển lễ: %s", nWeddingLevel),
		string.format("Loại hình: %s", Marry.MAP_LEVEL_NAME[nMapLevel])
	);
end
...
function tbMission:OpenRightUI(pPlayer, nRemainFrame)
	...
	Dialog:SetBattleTimer(pPlayer, "<color=green>Thời gian điển lễ: %s<color>\n", nRemainFrame);
...
function tbMission:UpdateAllRightUI()
	...
	local szMsg = string.format("<color=yellow>Số người tham dự: %s/%s<color>\n%s\n\n%s", 
