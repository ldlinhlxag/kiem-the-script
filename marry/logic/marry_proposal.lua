...
function Marry:AddQiuhun(pMale, pFemale)
	...
	pMale.AddSpeTitle(string.format("Tình nhân của %s", pFemale.szName), GetTime() + 7 * 60 * 60 * 24, "gold");
	pFemale.AddSpeTitle(string.format("Tình nhân của %s", pMale.szName), GetTime() + 7 * 60 * 60 * 24, "gold");
end
...
function Marry:DailyEvent()
	...
	local nDaily = me.GetTask(Marry.TASK_GROUP_ID, Marry.TASK_QINGHUA_DAILY);
	if nDaily > 0 then
...
function Marry:OnPlayerLogin()
	...
	if me.nSex == 0 then
		me.RemoveSpeTitle(string.format("tình nhân %s", szMatchName));
	else
		me.RemoveSpeTitle(string.format("tình nhân %s", szMatchName));
	end
