...
function tbBase:OpenSingleUi(pPlayer)
	...
	local szMsg = "<color=green>Thòi gian mở ải còn <color><color=white>%s<color> phút"
	...
	for _, pPlayer in ipairs(tbPlayer) do
		for _, pPlayer1 in ipairs(tbPlayer) do
			if (pPlayer.IsFriendRelation(pPlayer1.szName) == 1) then
				...
				pPlayer.Msg(string.format("Bạn và <color=yellow>%s<color> độ thân mật tằng %d điểm.", pPlayer1.szName, 50));
			end
		end
	end
end