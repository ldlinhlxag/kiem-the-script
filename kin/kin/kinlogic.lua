...
function Kin:CreateKin(anPlayerId, anStoredRepute, szKinName, nCamp, nCreateTime, tbStock)
	...
	for i, nPlayerId in ipairs(anPlayerId) do
		...
		if MODULE_GC_SERVER then
			...
			local szMsg = string.format("%s đã gia nhập gia tộc", KGCPlayer.GetPlayerName(nPlayerId));
	...
	cKin.SetTitleMan("Thành viên")
	cKin.SetTitleWoman("Thành viên")
...
function Kin:ParseHistory(tbRecord)
	...
	return string.format("%s："..tbParse.szFormat, os.date("%Y-%m-%d", tbRecord.nTime), unpack(tbRecord.tbContent));
end
...
function Kin:LogWeeklyTaskLevel()
	...
	for nTask = 1, 5 do
		...
		for nKinLevel = 0, 5 do
			...
			KStatLog.ModifyField("kinweeklytask", tbTaskName[nTask], tostring(nKinLevel) .. "等级", tostring(self.KinLevel_WeeklyTask[nTask][nKinLevel]));
			self.KinLevel_WeeklyTask[nTask][nKinLevel] = 0;
		end
	end
	...
	for nTask = 1, 5 do
		...
		for nPersonLevel = 0, 5 do
			...
			KStatLog.ModifyField("personweeklytask", tbTaskName[nTask], tostring(nPersonLevel) .. "Cấp", tostring(self.PersonalLevel_WeeklyTask[nTask][nPersonLevel]));
			self.PersonalLevel_WeeklyTask[nTask][nPersonLevel] = 0;
		end
	end
end
