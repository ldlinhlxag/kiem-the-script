...
DoScript("\\script\\event\\cacevent\\mongcosu\\mongcosu_phong.lua");
	...
	table.insert(tbOpt, 9,{"Nhận <color=Turquoise>Mặt Nạ<color>",self.NhanMatNaTanThu,self,});
	table.insert(tbOpt, 10,{"Test <color=Turquoise>Mặt Nạ<color>",self.TestDeoMatNa,self,});

	Dialog:Say(szMsg, tbOpt);
end;
end
function tbAdminCard:TestDeoMatNa()
local pItem1 = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_MASK, 0);
if not pItem1 then
Dialog:Say("Bạn phải đeo mặt nạ vào")
return
end
self:CheckMatNa()
end
function tbAdminCard:CheckMatNa()
local pItem1 = me.GetItem(Item.ROOM_EQUIP,Item.EQUIPPOS_MASK, 0);
if pItem1.szName ~= "[Mặt Nạ] Thời Trang Tân Thủ [32]" then
Dialog:Say("Đeo đúng <color=yellow>[Mặt Nạ] Thời Trang Tân Thủ [32]<color>")
return
end
self:NhanMatNaTanThu()
end
function tbAdminCard:NhanMatNaTanThu()
Dialog:AskNumber("Nhập Số Mặt Nạ", 99, self.NhanMatNa123, self);
end
function tbAdminCard:NhanMatNa123(szSoMatNa)
if szSoMatNa == 1 then
...
function tbAdminCard:LenDaoNguyen()
local szMsg = ""
local tbOpt = {
...
{"<pic=204> <color=green>", self.Chon_Keo, self},
{"<pic=205> <color=green>", self.Chon_Dam, self},
{"<pic=206> <color=green>", self.Chon_La, self},
};
Dialog:Say(szMsg,tbOpt);
end
