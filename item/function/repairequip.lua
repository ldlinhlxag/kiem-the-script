...
function Item:CommonRepair()					-- 程序回调接口：普通修理
	...
	if nPrice > 0 then
		KStatLog.ModifyAdd("jxb", "[消耗]装备修理", "总量", nPrice);
	end
	...
	if (it.nMaxDur <= Item.DUR_WARNING) then	-- 最大耐久太低，给出提示
		if (MathRandom(100) < 30) then			-- 30%的几率损坏
			it.nMaxDur = 0;
			it.nCurDur = 0;
			me.Msg("您的<"..it.szName..">已经损坏，你必须修复它的最大耐久才能继续使用。");
			return	1;
		end
	else
	...
	if (it.nMaxDur <= Item.DUR_WARNING) then
		me.Msg("您的<"..it.szName..">当前最大耐久已经降到了最低极限，它随时有可能损坏，您需要用<金犀>来修理，以恢复当前最大耐久。");
	end

	it.nCurDur = it.nMaxDur;	-- 进行修理
	local nDelta = nOldMax - math.floor(it.nMaxDur / 10);
	me.Msg("您的“"..it.szName.."”已经修复，最大耐久下降了"..nDelta.."点。");
	return	1;
...
function Item:SpecialRepair()					-- 程序回调接口：特殊修理
	...
	if (me.CostBindMoney(nPrice, Player.emKBINDMONEY_COST_REPAIR2) ~= 1) then
		if (me.nCashMoney + me.GetBindMoney() < nPrice) then
			...
			KStatLog.ModifyAdd("bindjxb", "[消耗]装备修理", "总量", nBindMoney);
			KStatLog.ModifyAdd("jxb", "[消耗]装备修理", "总量", nPrice - nBindMoney);
		end
	else
		KStatLog.ModifyAdd("bindjxb", "[消耗]装备修理", "总量", nPrice);
	end

	-- 修理程序
	it.nMaxDur = Item.DUR_MAX;					-- 补满最大耐久
	it.nCurDur = it.nMaxDur;					-- 补满当前耐久
	me.Msg("您的“"..it.szName.."”最大耐久已经修复。");
	return	1;
...
function Item:ItemRepair(pUseItem)				-- 程序回调接口：使用道具修理
	...
	it.nMaxDur = nMaxDur;
	it.nCurDur = it.nMaxDur;					-- 补满当前耐久
	me.Msg("您的“"..it.szName.."”最大耐久修复了"..nAddDur.."点。");
	return	1;
