...
function tbNpc:OnDialog()
	...
	local tbOpt = {
			{"Đưa vật phẩm", self.TakeInItem, self, nValue, NPC_TASKID[him.nTemplateId]},
			{"Kết thúc đối thoại"},
		}
	local szMsg = "Đưa vật phẩm";
	Dialog:Say(szMsg, tbOpt);
end;
...
function tbNpc:TakeInItem(nValue, nTaskId)
	local szMsg = "Đưa vật phẩm";
	if (nValue == 1) then
		szMsg = "Hãy đặt và 800 Ngũ Hành Hồn Thạch";
	end;
	if (nValue == 3) then
		szMsg = "Hãy đặt và 1600 Ngũ Hành Hồn Thạch"
	end; 
...
function tbNpc:OnOpenGiftOk(nValue, nTaskId, tbItemObj)
	...
	for _, pItem in pairs(tbItemObj) do
		if (self:ChechItem(pItem, REQUIRE_ITEM[nValue], tbItemList) ~= 1) then
			me.Msg("Vật phẩm không chính xác!");
			return 0;
		end;
	end
	...
	if (bResult) then
		me.Msg("Số lượng vật phẩm không đủ!");
		return 0;
	end;
