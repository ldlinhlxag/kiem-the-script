...
function tbGift:OnSwitch(pPickItem, pDropItem, nX, nY)
	...
	-- 隻能放入辨認過的藏寶圖
	local nIdentify = pDropItem.GetGenInfo(TreasureMap.ItemGenIdx_IsIdentify);	-- 是否是辨認過的藏寶圖
	if (nIdentify ~= 1) then
...
function tbGift:OnOK()
	...
	local nIdentify = pTreasureMap.GetGenInfo(TreasureMap.ItemGenIdx_IsIdentify);	-- 是否是辨認過的藏寶圖
	if (nIdentify ~= 1) then
		Dialog:SendInfoBoardMsg(me, "<color=red>La bàn chỉ phân biệt những Tàng Bảo Đồ đã đọc qua!<color>");
		return;
	end	
	
	local nTreasureId		= pTreasureMap.GetGenInfo(TreasureMap.ItemGenIdx_nTreaaureId); -- 所對應寶藏的編號
