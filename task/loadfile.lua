...
--Task.tbFiltTask = {
--Task={
--	["00000000000001CF"] = 1,		--Í¬°é
--	["00000000000001D0"] = 1,		--Í¬°é
--	["00000000000001D1"] = 1,		--Í¬°é
--	["00000000000001D4"] = 1,		--¸ß¼¶ÃØ¼®
--},
--Sub={
--	["0000000000000290"] = 1,		--Í¬°é
--	["0000000000000291"] = 1,		--Í¬°é
--	["0000000000000292"] = 1,		--Í¬°é
--	["0000000000000295"] = 1,		--¸ß¼¶ÃØ¼®
--},
--}
--end
...
function Task:ReadParams(tbXml)
	...
	for _, tb in ipairs(tbXml.children) do
		...
		local szValue;
		for _, tb in ipairs(tb.children) do--這裡不會有多個childern吧
...
function Task:LoadTask(nTaskId)
	...
	if self.tbFiltTask.Task[szTaskFileName] then
		return;
	end
	if (not tbXml) then
		return;
		...
		elseif (tb.name == "Managed") then
			...
			for _, tb in ipairs(tb.children) do
				if (tb.name == "Sub") then
					...
					local nReferId		= tonumber(tb.attrib.id, 16); -- 引用子任務Id
					...
					tbReferData.tbVisable	= Task:ReadFunctions(tbAttrib.children[2]);	-- 讀取可見條件
					tbReferData.tbAccept	= Task:ReadFunctions(tbAttrib.children[3]); -- 讀取可接條件
					-- Xml:	Task\Managed\Sub\Attribute\Parameter
					local tbParams	= self:ReadParams(tbAttrib.children[1]); -- 讀取參數
					...
					if (tbParams[2] ~= "") then
						...
						local tbAcceptItemId = Lib:Str2Val(tbParams[2]);
						tbReferData.nParticular = tbAcceptItemId[3];
						if (tbReferData.nAcceptNpcId > 0 and tbReferData.nParticular) then -- 不能同時物品和Npc都可接
							print("[Task:Warning] 物品和Npc可同時觸發:"..tbReferData.szName);
						end
					end;
					if (tbParams[3] == "") then
						tbParams[3] = "{}";
			...
			for _, tb in ipairs(tb.children[1].children) do
				...
				elseif (tb.name == "Target") then
					...
					if (tb.children[1]) then
						local nNewWorldFlag = 0; -- 傳送目標隻能第一個有效
						for _, tb in ipairs(tb.children[1].children) do
							if (tb.name == "Grid") then
...
function Task:ParseRefSubDesc(strSrc)
	...
	while true do
		...
		if (not nNextStepStartIdx) then -- 沒有下一個<step>標記
...
function Task:ParseDialog(szAllMsg)
	...
	tbRetStepMsg.szMsg = nil; -- 若沒分步驟則會填充此字段
	tbRetStepMsg.tbSetpMsg = {}; -- 若分了步驟則此表被填充
	
	if (#tbAllStepMsg == 1) then -- 沒有分步驟，即所有步驟都使用同樣的對話。
		tbRetStepMsg.szMsg = tbAllStepMsg[1];
	else --分有步驟，得到當前步驟對話信息
