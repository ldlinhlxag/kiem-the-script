...
function tbCampBase:OnEnd(nResult, szWinMsg)	
	...
	for _, pPlayer in pairs(tbPlayerList) do
		...
		pPlayer.Msg(string.format("Xếp hạng của bạn là ：<color=green>%d<color>, Bạn nhận được ：<color=yellow>%d<color> điểm tích lũy", tbBattleInfo.nListRank, tbBattleInfo.nBouns));
	end
end
...
function tbCampBase:OnProtectFlag(tbFlagPlayer)
	...
	local szFirMsg = string.format("%s Quân %s <color=green>%s<color>Hộ tống chiến kì thành công", Battle.NAME_CAMP[tbFlagPlayer.tbCamp.nCampId], Battle.NAME_RANK[tbFlagPlayer.nRank], tbFlagPlayer.pPlayer.szName);
	local szSecMsg = string.format("Hiện nay bên %s Đã hộ tống thành công： <color=red>%d<color> chiến kì", Battle.NAME_CAMP[tbFlagPlayer.tbCamp.nCampId], tbFlagPlayer.tbCamp.nFlags);
