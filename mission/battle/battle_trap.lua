...
function tbCampBase:_BaseToOuter(pPlayer, szPosName)
	...
	if (nRemainTime > 0) then
		Dialog:Say(string.format("Bạn đã mệt rồi,Hãy Nghỉ ngơi sau %d giây", nRemainTime));
		return;
	end
...
function tbCampBase:OnTrap_OuterToBase(pPlayer)
	...
	if (tbBattleInfo.tbCamp.nCampId ~= self.nCampId) then
		tbBattleInfo.pPlayer.Msg("Phía trước có rất nhiều cạm bẫy của quân địch,cẩn thận vẫn hơn!");
		return;
	end
