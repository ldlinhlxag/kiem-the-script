...
-- 杀不同Npc获得的积分
tbRuleBase.BOUNS_KILL_NPC	= { 1, 1, 5, 5, 20, 20, 50, 50, 500, 1500 };
...
function tbRuleBase:GetKillNpcBoardMsg(nRankId, nNpcBouns, pNpc)
	...
	elseif (9 <= nRankId) then
		...
		nMidMsg = 2;
		szMsg 	= string.format("你浴血奋战，斩杀<color=yellow>%s<color>，获得奖励<color=yellow>%d<color>积分。", szNpcName, nNpcBouns);
		if (9 == nRankId) then
			nMidMsg = 1;
		end
	end
...
function tbRuleBase:OnDeath_Yuanshuai(nCampId, pNpc)
	...
	if (szKillerName) then
		...
		local szMsg = string.format("<color=yellow>%s<color>军 <color=yellow>%s<color> 小队奋勇斩杀<color=yellow>%s军元帅<color>, %s军获得奖励积分10万。", Battle.NAME_CAMP[nWinCampId], szKillerName, Battle.NAME_CAMP[nLoseCampId], Battle.NAME_CAMP[nWinCampId]);
...
function tbRuleBase:TryAddMarshal(nCampId, bForce)
	...
	local szMsg	= string.format("%s军元帅率兵出阵杀敌，%s军将士速速前往，护卫元帅！", szCampName, szCampName);
...
function tbRuleBase:OnTimer_AddDaJiang()
	for nCampId, tbMapInfo in pairs(self.tbMapInfoCamp) do		
		...
		local szCampName	= Battle.NAME_CAMP[nCampId];
		local szMsg			= string.format("%s军大将率兵出阵杀敌，%s军将士速速前往，护卫大将！", szCampName, szCampName);
...
function tbRuleBase:AddProtectBouns(tbCamp, tbBoss, nRankId, nCampId, nDistanceSqure)
	...
	local szMsg		= string.format("你护卫%s军%s有功，获得奖励<color=yellow>%d<color>积分。",
...
function tbRuleBase:GiveKillBossCampBouns(nCampId, nRankId)
	...
	local szMsg	= string.format("%s军%s已被我军将士奋勇击退，你获得奖励<color=yellow>%d<color>积分。",
						tbOppCamp.szCampName, Battle.NAME_RANK[nRankId], nAddBouns);
	if (10 == nRankId) then
		szMsg = szMsg .. "本方总分增加10万。";
	end
	self.tbMission:BroadcastMsg(nCampId, szMsg);
