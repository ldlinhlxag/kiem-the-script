...
local tbRuleBase	= Battle:GetRuleClass(3, "Hộ Tống chiến kì");

-- 杀不同Npc获得的积分
tbRuleBase.BOUNS_KILL_NPC	= { 1, 1, 5, 5, 20, 20, 50, 50, 0, 0 };
-- 玩家护旗成功奖励积分
...
function tbRuleBase:AddFlag(nCampId)
	...
	local szMsg		= string.format("<color=yellow>%s<color> Chiến Kì tại <color=green>(%d, %d)<color>。", Battle.NAME_CAMP[nCampId], tbPos[2] / 8, tbPos[3] / 16);
...
function tbRuleBase:DeletePlayerFlag(pPlayer)
	...
	if ((2 == self.tbMission.nState) and (1 == tbBattleInfo.bHaveFlag) and (tbBattleInfo.pPlayer.nId == tbBattleInfo.tbCamp.nPlayerIsFlag)) then
		...
		local szMsg		= string.format("%s Chiến kì tại (%d, %d)。", Battle.NAME_CAMP[nCampId], nPosX / 8, nPosY / 16);
		self.tbMission:BroadcastMsg(szMsg);
...
function tbRuleBase:GetKillNpcBoardMsg(nRankId, nNpcBouns, pNpc)
	local szMsg		= "";
	szMsg = string.format("Bạn Bị bị đánh Trọng Thương bởi %s", Battle.NAME_RANK[nRankId]);
	return szMsg, 0;
end
				...
				if (1 == self.nDuringProtect_TimeFlag) then
					...
					if (tbPos and tbPos[1]) then
						...
						if (nDx*nDx + nDy*nDy <= nDistanceSqure) then
							...
							szMsg		= string.format("Bảo vệ cờ thành công,Tặng thưởng <color=yellow>%d<color> điểm tích lũy。", nAddBouns);
							pPlayer.Msg(szMsg);
						end
					end
				end
