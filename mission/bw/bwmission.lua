...
--		KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, szMsg);
	...
	if (nRestMin > 0) and (nRestSec == 0) then
		szMsg = string.format("擂台赛入场剩余时间：<color=yellow>%d<color>分钟。", nRestMin);
		self:BroadcastMsg(szMsg);
		
	elseif (nRestMin == 0 and nRestSec > 0) then
		szMsg = string.format("擂台赛入场剩余时间：<color=yellow>%d<color>秒。",nRestSec);
		self:BroadcastMsg(szMsg);
	end;
	return szMsg;
end;
...
function tbTest:ShowTimeInfo(pPlayer)
	...
	local szMsgFormat		= "<color=green>剩余时间：<color> <color=white>%s<color>";
