...
function DragonBoat:AddHonor(pPlayer, nHonor)
	...
	pPlayer.Msg(string.format("Bạn nhận được <color=yellow>%s điểm vinh dự đua thuyền rồng<color>", nHonor));
...
function DragonBoat:OnPlayerType3(nGroup, nPoint)
	...
	if nGiveRank == 0 then
		me.Msg("Trên đường đua đã không còn đối thủ để tấn công!");
		Dialog:SendBlackBoardMsg(me, "Trên đường đua đã không còn đối thủ để tấn công!");
	else
...
function DragonBoat:OnPlayerBaseType1(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("Bạn tình cờ phát hiện được Bí Thuật Thuyền Rồng %s!", KFightSkill.GetSkillName(tbSkill[1])));
	me.Msg(string.format("<color=blue>Bạn tình cờ phát hiện được Bí Thuật Thuyền Rồng %s!<color>", KFightSkill.GetSkillName(tbSkill[1])));	
end
...
function DragonBoat:OnPlayerBaseType2(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("Bỗng đâu, thuyền rồng của bạn bị Bí Thuật %s quấy nhiễu!", KFightSkill.GetSkillName(tbSkill[1])));
	me.Msg(string.format("<color=blue>Bỗng đâu, thuyền rồng của bạn bị Bí Thuật %s quấy nhiễu!<color>", KFightSkill.GetSkillName(tbSkill[1])));	
end
...
function DragonBoat:OnPlayerBaseType3(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("Bạn tình cờ vớt được %s!", KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4])));
	me.Msg(string.format("<color=blue>Bạn tình cờ vớt được %s!<color>", KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4])));		
end
...
function DragonBoat:OnPlayerBaseType4(pCastPlayer, tbPlayerInfo)
	...
	if pCastPlayer.nId == pPlayer.nId then
		local szMsg = string.format("Thật xui xẻo, bạn bị tấn công bởi chính Thiên Phạt %s mà mình phóng thích.", KFightSkill.GetSkillName(tbSkill[1]));
		pPlayer.Msg(string.format("<color=blue>%s<color>", szMsg));
		Dialog:SendBlackBoardMsg(pPlayer, szMsg);
		return 0;
	end
	
	local szMsg1 = string.format("Thật xui xẻo, bạn bị Thiên Phạt do %s tấn công %s!", pCastPlayer.szName, KFightSkill.GetSkillName(tbSkill[1]));	
	local szMsg2 = string.format("Bạn phóng thích Thiên Phạt tấn công %s - %s!", pPlayer.szName, KFightSkill.GetSkillName(tbSkill[1]));	
