...
function Wlls:SetRankData(tbData, nMacthType, szTitle, nRankParam1, nRankParam2, nRankParam3, nRankParam4)
	...
	for nRank, tbLeague in ipairs(tbData) do
		...
		local tbMemberInfo = {
			...
			szTxt5 = string.format("比赛时间:%s", Lib:TimeFullDesc(tbLeague.nTime)),
			szTxt6 = "",
...
function Wlls:LeagueRank(nFinalType, nUpdateNews)
	...
	if tbMacthCfg.nMapLinkType == Wlls.MAP_LINK_TYPE_RANDOM then
		...
		if (MODULE_GC_SERVER) then	
			--当届前10名入榜.
			Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_PRIM], tbMacthCfg.nMapLinkType, "本届初级联赛榜", 0, Ladder.LADDER_CLASS_WLLS, 1, 0);
			Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_ADV], tbMacthCfg.nMapLinkType, "本届高级联赛榜", 0, Ladder.LADDER_CLASS_WLLS, 2, 0);
	...
	if tbMacthCfg.nMapLinkType == Wlls.MAP_LINK_TYPE_SERIES then
		...
		if (MODULE_GC_SERVER) then	
			...
			for nSereis in pairs(self.RankLeagueList[Wlls.MACTH_PRIM]) do
				Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_PRIM][nSereis], tbMacthCfg.nMapLinkType, "本届初级联赛榜", 0, Ladder.LADDER_CLASS_WLLS, 1, nSereis);
			end
			
			for nSereis in pairs(self.RankLeagueList[Wlls.MACTH_ADV]) do
			    Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_ADV][nSereis], tbMacthCfg.nMapLinkType, "本届高级联赛榜", 0, Ladder.LADDER_CLASS_WLLS, 2, nSereis);
	...
	if tbMacthCfg.nMapLinkType == Wlls.MAP_LINK_TYPE_FACTION then
		...
		if (MODULE_GC_SERVER) then	
			...
			for nFaction in pairs(self.RankLeagueList[Wlls.MACTH_PRIM]) do
				Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_PRIM][nFaction], tbMacthCfg.nMapLinkType, "本届初级联赛榜", 0, Ladder.LADDER_CLASS_WLLS, 1, nFaction);
			end
			
			for nFaction in pairs(self.RankLeagueList[Wlls.MACTH_ADV]) do
			    Wlls:SetRankData(self.RankLeagueList[Wlls.MACTH_ADV][nFaction], tbMacthCfg.nMapLinkType, "本届高级联赛榜", 0, Ladder.LADDER_CLASS_WLLS, 2, nFaction);
				local szFaction = Wlls.LADDER_FACTIONNAME[nFaction];
				-- Player:GetFactionRouteName(nFaction);
			end
			GlobalExcute{"Ladder:RefreshLadderName"};
