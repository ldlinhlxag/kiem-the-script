	...
	for nId, tbParam in ipairs(tbFile) do
		if nId > 1 then
			...
			if (nSession) then
				...
				Wlls.SEASON_TB[nSession] = {nLeagueType, szAwardFile, nAdvRank, szDesc, nPreMaxLeague};
	...
	for nType, szClassName in pairs(Wlls.MACTH_TYPE) do
		...
		if tbFileName[nType] then
			...
			if not tbTypeFile then
				print("[Võ Lâm Liên Đấu] đọc file bị lỗi, file không tồn tại", tbFileName[nType]);
				return
			end
			...
			if not Wlls.MacthType[szClassName].szName then
				Wlls.MacthType[szClassName].szName = "[Chưa điền hình thức]";
			end
			if not Wlls.MacthType[szClassName].szDesc then
				Wlls.MacthType[szClassName].szDesc = "[Chưa điền miêu tả]";
			end
		...
		if tbFileTrap[nType] then
			...
			if not tbTypeFile then
				print("[Võ Lâm Liên Đấu] đọc file bị lỗi, file không tồn tại", tbFileTrap[nType]);
				return
			end
