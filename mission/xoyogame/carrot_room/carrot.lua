...
function RoomCarrot:UpdateTargetinfo()
	self:SetTagetInfo(1, 
		string.format("Ta có %d củ cải\nHọ có %d củ cải", self.tbTeam[1].nCount, self.tbTeam[2].nCount));

	self:SetTagetInfo(2, 
		string.format("Ta có %d củ cải\nHọ có %d củ cải", self.tbTeam[2].nCount, self.tbTeam[1].nCount));
end
...
function RoomCarrot:FinishMsg()
	if self.tbTeam[1].nCount == self.tbTeam[2].nCount then
		self:MovieDialog(-1, "Lực lượng ngang nhau, cân sức ngang tài.");
		return
	end
	...
	self:MovieDialog(nWinTeam, "Chúng ta quả nhiên thị kỹ cao một bậc.");
	self:MovieDialog(nLoseTeam, "Mặc dù bại do vinh quang, lần sau tiếp tục nỗ lực.");
end
