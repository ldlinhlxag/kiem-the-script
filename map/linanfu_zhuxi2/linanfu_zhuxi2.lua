...
-------------- 定义特定地图回调 ---------------
local tbTest = Map:GetClass(487); -- 地图Id
...
function tbTestTrap:OnPlayer()	
	...
	if (task_value == 1) then 	
		me.NewWorld(488,1530,3956);	-- 传送,[地图Id,坐标X,坐标Y]
		me.SetFightState(1);
		return;
	elseif (task_value == 2) then 	
		me.NewWorld(491,1530,3956);	-- 传送,[地图Id,坐标X,坐标Y]
		me.SetFightState(1);
		return;
	elseif (task_value == 3) then 	
		me.NewWorld(492,1530,3956);	-- 传送,[地图Id,坐标X,坐标Y]
		me.SetFightState(1);
		return;
	else
