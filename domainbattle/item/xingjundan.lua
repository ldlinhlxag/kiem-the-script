...
function tbXingJunDan:OnUse()
	if (me.GetNpc().GetRangeDamageFlag() ~= 1) then
		me.Msg("Hành Quân Đơn trong thời kỳ chinh chiến mới có thể sử dụng!");
		return 0;
	end
	for i = 1, #self.SKILLID do
		me.AddSkillState(self.SKILLID[i], self.LEVEL[it.nLevel], 2, self.LASTTIME * Env.GAME_FPS, 1, 0, 1);
	end
	return 1;
end
