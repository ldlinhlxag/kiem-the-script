...
function tbChongZhi:OnJiHuoGetRepute(nFlag)
	if self:CheckISCanGetRepute() == 1 then
		Dialog:Say("你当前角色已经成功激活过领取江湖威望资格。", {{"返回上层", self.OnDialog, self},{"Kết thúc đối thoại"}});
		return 0;
	end
	
	if self:CheckIsSetExt() == 1 then
		Dialog:Say("你的帐号下其他角色已经激活了领取江湖威望资格，一个帐号下只允许一个角色激活。", {{"返回上层", self.OnDialog, self},{"Kết thúc đối thoại"}});
		return 0;
	end
	...
	Dialog:Say("你的角色成功激活了领取江湖威望资格，请充值后领取本月赠送江湖威望。", {{"返回上层", self.OnDialog, self},{"Kết thúc đối thoại"}});
end

function tbChongZhi:GetRepute()
	--local nResult, nCount = self:Check();
	if self:CheckISCanGetRepute() == 0 then
		Dialog:Say("你的角色没有激活领取江湖威望资格。", {{"返回上层", self.OnDialog, self},{"Kết thúc đối thoại"}});
		return 0;
	end
	...
	if nResultRepute < 0 then
		...
		Dialog:Say(szMsg, {{"返回上层", self.OnDialog, self},{"Kết thúc đối thoại"}});
		return 0;
	end
	
	if nResultRepute == 0 then
		Dialog:Say(string.format("按你的%s额度，您本周已经领取了%s点江湖威望了。", IVER_g_szPayName, nSumRepute), {{"返回上层", self.OnDialog, self},{"Kết thúc đối thoại"}});
		return 0;		
	end
	...
	Dialog:Say(string.format("礼官：成功获得了%d点江湖威望。", nResultRepute), {{"返回上层", self.OnDialog, self},{"Kết thúc đối thoại"}});
end
