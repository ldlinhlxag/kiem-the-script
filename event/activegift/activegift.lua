...
function ActiveGift:AddActive(nGrade, nId)
	...
	if nLastGrade < self.nMaxActiveGrade then
		...
		me.Msg("Nhận Được <color=yellow>"..nTotalGrade-nLastGrade.."<color> Điểm Hoạt Động.");		--完成提示
		--log
		...
		if nTotalGrade >= self.nMaxActiveGrade then
			me.Msg("Hoạt động trong ngày của bạn đã đạt cao nhất.");		--完成提示
		end
	else
		me.Msg("Hoạt động trong ngày của bạn đã đạt đến cao nhất không còn tích lũy.");		--完成提示
		return;
	end
...
function ActiveGift:GetActiveAward(nId)
	...
	if self.tbAwardName[nId] then
		me.SendMsgToFriend(string.format("Bạn bè của bạn [<color=yellow>%s<color>] đạt được một <color=yellow>%s<color>，Điểm Hoạt động tăng <color=yellow>%s<color>", me.szName, self.tbAwardName[nId], me.GetTask(self.nTaskGroupId, self.nActiveGrade)));
	end
	Dbg:WriteLog("ActiveGift", string.format("người chơi %s id Activegift %s",me.szName,nId));
end
