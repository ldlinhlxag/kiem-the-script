...
function tbItem:GetItem(pPlayer, tbitem)
	...
	if tbitem.nFriendMsg == 1 then
		pPlayer.SendMsgToFriend("您的好友[<color=yellow>"..pPlayer.szName.."<color>]打开"..tbitem.szDesc.."获得了<color=yellow>"..tbitem.szName.."<color>。");
	end
...
function tbItem:CheckItemFree(pPlayer, nCount)
	if pPlayer.CountFreeBagCell() < nCount then
		local szAnnouce = "您的背包空间不足，请留出"..nCount.."格空间再试。";
		pPlayer.Msg(szAnnouce);
		return 0;
	end
	return 1;
