...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("mongcosu_nhat");
...
function tbQinshihuangBoss:DeathLoseItem(tbLoseItem)
	...
	local szMsg = "<color=green>Mông Cổ Sứ (Nhật) rơi vật phẩm:<color>\n";
	local tbList = {};
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "mongcosu_nhat");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "mongcosu_nhat");
			end
		end
	end
	...
	local szMsg = string.format("Tổ đội của <color=green>%s<color> tại tầng Tàn Tích Cung A Phòng đánh thắng Mông Cổ Sứ (Nhật)!!!", pPlayer.szName);
	KDialog.NewsMsg(0, Env.NEWSMSG_COUNT, szMsg);
			GlobalExcute({"Dialog:GlobalNewsMsg_GS","<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: đánh bại Mông Cổ Sứ (Nhật)"});
	KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: đánh bại Mông Cổ Sứ (Nhật)");
	KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> <color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: đánh bại Mông Cổ Sứ (Nhật)");	
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Mông Cổ Sứ (Nhật) nói: "..szMsg);
