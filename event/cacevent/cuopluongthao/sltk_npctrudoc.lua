...
-- 秦始皇boss
local tbQinshihuangBoss	= Npc:GetClass("sltk_npctrudoc");
...
function tbQinshihuangBoss:DeathLoseItem(tbLoseItem)
	...
	local szMsg = "<color=green>Trụ (Độc) rơi vật phẩm:<color>\n";
	local tbList = {};
...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
	...
	if nTeamId == 0 then
		pPlayer.AddKinReputeEntry(5, "sltk_npctrudoc");
	else
		...
		for i, nPlayerId in pairs(tbPlayerId) do
			...
			if (pTeamPlayer and pTeamPlayer.nMapId == him.nMapId) then
				pTeamPlayer.AddKinReputeEntry(5, "sltk_npctrudoc");
			end
		end
	end
	...
	local szMsg = string.format("Tổ đội của <color=green>%s<color> tại tầng Chiến Trường Cổ hạ Trụ (Độc)!!!\nCác nhân sĩ mau tới Chiến Trường Cổ để lấy lương thảo !", pPlayer.szName);
	GlobalExcute({"Dialog:GlobalNewsMsg_GS","<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ Trụ (Độc) nhận được <color=cyan>10 Hòa Thị Bích , 10 Đồng Tiền Vàng, 100tr EXP , 1 Huyền Tinh 9 , 5 Huân Chương Vàng, 5 Phôi Giáp, 5 Củi<color>"});
	KDialog.NewsMsg(1, Env.NEWSMSG_COUNT,"<color=red>[Anh Hùng] <color><color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ Trụ (Độc) nhận được <color=cyan>10 Hòa Thị Bích , 10 Đồng Tiền Vàng, 100tr EXP , 1 Huyền Tinh 9 , 5 Huân Chương Vàng, 5 Phôi Giáp, 5 Củi<color>");
	KDialog.MsgToGlobal("<color=red>[Anh Hùng]<color> <color=yellow><color=Turquoise>"..pPlayer.szName.."<color>: hạ Trụ (Độc) nhận được <color=cyan>10 Hòa Thị Bích , 10 Đồng Tiền Vàng, 100tr EXP , 1 Huyền Tinh 9 , 5 Huân Chương Vàng, 5 Phôi Giáp, 5 Củi<color>");	
	KDialog.NewsMsg(0, Env.NEWSMSG_COUNT, szMsg);
	self:BroadCast(szMsg);
	pPlayer.AddStackItem(18,1,377,1,nil,10) -- 10 Hòa Thị Bích
	pPlayer.AddStackItem(18,10,11,2,nil,10) -- 10 ĐTV
	pPlayer.AddExp(100000000) -- 100tr EXP
	pPlayer.AddStackItem(18,1,1,9,nil,1) -- 1 HT9
	pPlayer.AddStackItem(18,1,20319,1,nil,5) --  5 Huân Chương Vàng
	pPlayer.AddStackItem(18,1,20335,1,nil,5) --  5 Phôi Giáp
	pPlayer.AddStackItem(18,1,20344,1,nil,5) -- 5 Củi
	-- 股份和荣誉
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
	...
	if nLifePercent == 80 then
		...
		if Boss.Qinshihuang:GetBossStep() == 0 then
			...
			pNpc.SendChat(szMsg);
			Boss.Qinshihuang:Broadcast("Trụ (Độc) nói: "..szMsg);
