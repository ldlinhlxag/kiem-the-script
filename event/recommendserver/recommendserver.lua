...
tbServer.DIALOGTXT 	=
{
	...
	szAward 	= nil,
	szAbout		= "首先，必须在10级以前在活动推广员处登记确认能领取入驻奖励。然后您在20天内达到30，40，50，60级时，就能领取相应的奖励",
	...
	szNoFreeBag	= "对不起,Túi của bạn không đủ chỗ trống"
}
tbServer.AWARD = nil;
...
tbServer.__szAward_1 = string.format("欢迎您入驻本推荐服务器，在您达到对应等级时，可以在各城市新手村活动推广员处领取以下奖励：\n   30级：1000绑定%s\n   40级：12格背包\n   50级：3000绑定%s\n   60级：6000绑定%s\n   注意：您只有在20天内(<color=yellow>%%s<color>前)达到上述等级后才可以获得相应奖励", IVER_g_szCoinName, IVER_g_szCoinName, IVER_g_szCoinName)
...
function tbServer:GetAward(nLevel)
	...
	if self.AWARD[nLevel].tbItem then
		...
		if pItem then
			pItem.Bind(1);
			me.Msg(string.format("恭喜您获得了一个<color=yellow>%s<color>", pItem.szName))
