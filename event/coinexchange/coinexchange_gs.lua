...
function CoinExchange:ExchangePayerMaxIsSusscess(nPlayerId, nSuccess)
	...
	if nSuccess == 1 then
		...
		Dialog:Say("Bạn đã nhận phúc lợi tuần này, hãy quay lại vào tuần sau.");
	end
	return 0;
end
...
function CoinExchange:CanExchange(pPlayer)
	...
	if nPrestige == 0 then
		return -1, "Hệ thống đang bận hãy thử lại sau."
	end
	...
	if nPrestige > pPlayer.nPrestige then
		return 0, "Uy danh của bạn không đủ "..nPrestige.." điểm, không thể nhận phúc lợi."
	end
	...
	if nLastXchgWeek == nThisWeek then
		return 0, "Tuần này bạn đã nhận phúc lợi rồi."
	end
	
	return 1, "Đã nhận phúc lợi"
end
