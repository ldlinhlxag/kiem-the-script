...
function EventKind:OnDialog(szMsgFlag)
	if not szMsgFlag then
		...
		local szMsg 	= EventManager.tbFun:GetParam(self.tbEventPart.tbParam, "szMsg")[1] or "Xin chào, tôi có thể giúp gì cho  bạn ?";
		...
		for nId, szSelect in ipairs(tbSelect) do
			table.insert(tbOpt, {szSelect, self.OnDialog, self, tbMsg[nId] or "Xin chào, tôi có thể giúp gì cho  bạn ?"});
		end
