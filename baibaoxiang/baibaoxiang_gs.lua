...
function Baibaoxiang:CheckAllowGetAward(pPlayer, nType)
	...
	if nType == 1 then
		...
		if nAwardType == 1 or nAwardType == 5 then
			if pPlayer.CountFreeBagCell() < nAwardNum + nSpaceGC then
				Dialog:SendBlackBoardMsg(pPlayer, "Hành trang kh?ng ?? ch? tr?ng, h?y s?p x?p và th? l?i.");
				return 0;
			end
		...
		elseif nAwardType == 3 then
			if nAward * nAwardNum + pPlayer.nCashMoney > pPlayer.GetMaxCarryMoney() then
				Dialog:SendBlackBoardMsg(pPlayer, "S? ti?n ???c mang theo v??t quá gi?i h?n cho phép, h?y s?p x?p và th? l?i.");
				return 0;
			end
		end
	...
	elseif nType == 2 then
		...
		if pPlayer.CountFreeBagCell() < nSpace + nSpaceGC then
			local szMsg = string.format("Hành trang c?a b?n kh?ng ?? ch?, C?n %s ? tr?ng ?? nh?n th??ng", nSpace + nSpaceGC);
...
function Baibaoxiang:GetAward(pPlayer, nType)
	...
	if nType == 1 then
		...
		if nAwardType == 1 then
			...
			if nAwardLevel >= 4 then
				pPlayer.SendMsgToFriend("H?o h?u c?a b?n [<color=yellow>"..pPlayer.szName.."<color>] nh?n ???c t? bách b?o r??ng <color=yellow>"
					..nAwardNum.."<color> huy?n tinh "..nAward.."");
				Player:SendMsgToKinOrTong(pPlayer, "Nh?n ???c t? bách b?o r??ng "..nAwardNum.." huy?n tinh "..nAward..".", 0);
			end
			
			-- log
			self:_Log(pPlayer, "Nh?n ???c" .. nAward .. "huy?n tinh" .. nAwardNum .. "mot");
		...
		elseif nAwardType == 2 then
			...
			if nAwardLevel >= 4 then
				pPlayer.SendMsgToFriend("H?o h?u c?a b?n [<color=yellow>"..pPlayer.szName.."<color>] nh?n ???c t? bách b?o r??ng <color=yellow>"
					..(nAward * nAwardNum).."<color> tinh ho?t l?c");
				Player:SendMsgToKinOrTong(pPlayer, " nh?n ???c t? bách b?o r??ng "..(nAward * nAwardNum).." tinh ho?t l?c?", 0);
			end
			
			-- need channel msg 
			pPlayer.Msg(string.format("B?n nh?n ???c <color=yellow>%s<color> tinh l?c", nAward * nAwardNum));
			pPlayer.Msg(string.format("B?n nh?n ???c <color=yellow>%s<color> ho?t l?c", nAward * nAwardNum));
			
			-- log
			self:_Log(pPlayer, "Nh?n ???c" .. (nAward * nAwardNum) .. "tinh ho?t l?c");
		...
		elseif nAwardType == 3 then
			...
			if nAwardLevel >= 4 then
				pPlayer.SendMsgToFriend("H?o h?u c?a b?n [<color=yellow>"..pPlayer.szName.."<color>] nh?n ???c t? bách b?o r??ng<color=yellow>"
					..(nAward * nAwardNum).."<color> l??ng b?c!");
				Player:SendMsgToKinOrTong(pPlayer, " nh?n ???c t? bách b?o r??ng "..(nAward * nAwardNum).." l??ng b?c!", 0);
			end	
			
			-- log
			self:_Log(pPlayer, "Nh?n ???c" .. (nAward * nAwardNum) .. "l??ng b?c");
		...
		elseif nAwardType == 4 then
			...
			if nAwardLevel >= 4 then
				...
				Player:SendMsgToKinOrTong(pPlayer, " nh?n ???c t? bách b?o r??ng "..(nAward * nAwardNum)..string.format(" %s", IVER_g_szCoinName), 0);
			end	
			
			-- log
			self:_Log(pPlayer, "Nh?n ???c" .. (nAward * nAwardNum) .. "??ng khóa");
		...
		elseif nAwardType == 5 then
			...
			pPlayer.SendMsgToFriend("H?o h?u c?a b?n [<color=yellow>"..pPlayer.szName.."<color>] nh?n ???c t? bách b?o r??ng <color=yellow>"
				..(nAward * nAwardNum).."<color> r??ng");
			Player:SendMsgToKinOrTong(pPlayer, " nh?n ???c t? bách b?o r??ng "..(nAward * nAwardNum).." r??ng", 0);
					
			-- log
			self:_Log(pPlayer, "Nh?n ???c" .. (nAward * nAwardNum) .. "r??ng");
		end
	...
	elseif nType == 2 then
		...
		if nAwardLevel >= 4 then
			pPlayer.SendMsgToFriend("H?o h?u c?a b?n [<color=yellow>"..pPlayer.szName.."<color>] nh?n ???c t? bách b?o r??ng <color=yellow>"
				..(nAward * nAwardNum).."<color> V? sò vàng!");
			Player:SendMsgToKinOrTong(pPlayer, " nh?n ???c t? bách b?o r??ng "..(nAward * nAwardNum).." V? sò vàng !", 0);
		end
				
		-- log
		self:_Log(pPlayer, "Nh?n ???c" .. (nAward * nAwardNum) .. "v? sò vàng");		
...
function Baibaoxiang:GetCoin_GS(nPlayerID, nCoin)
	...
	-- channel
	local szMsg = string.format("Ng??i ch?i <color=green>%s<color> nh?n ???c thái tri t? bách b?o r??ng %s v? sò vàng ! Th?t may m?n.", pPlayer.szName, nCoin);
	KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, szMsg);
	
	pPlayer.SendMsgToFriend("H?o h?u c?a b?n [<color=green>"..pPlayer.szName.."<color>] nh?n ???c thái tri t? bách b?o r??ng <color=yellow>"
		..nCoin.."<color> v? sò vàng");
		
	Player:SendMsgToKinOrTong(pPlayer, " nh?n ???c thái tri t? bách b?o r??ng "..nCoin.." v? sò vàng", 0);
	...
	local szLog = "Nhan ???c t? bách b?o r??ng" .. nCoin .. "v? sò vàng, v? sò vàng còn l?i" .. nLeft;
...
function Baibaoxiang:_Log(pPlayer, szLog)
	...
	-- log
	Dbg:WriteLog("Baibaoxiang", "Bách b?o r??ng", pPlayer.szAccount, pPlayer.szName, szLog);
			
	-- service log
	pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, "S? d?ng bách b?o r??ng" .. szLog);	
end
...
function Baibaoxiang:UpdateHelpTable(pPlayer, nCoin)
	...
	local szTxt = "<color=cyan>" .. szDate .. "\n" .."<color=yellow>" .. pPlayer.szName 
		.. " <color=green>nh?n ???c thái tri t? bách b?o r??ng " .. "<color=yellow> " .. nCoin 
		.. " <color=green>" .. " v? sò vàng!<color>";
	...
	-- call addnews
	Task.tbHelp:AddDNews(Task.tbHelp.NEWSKEYID.NEWS_BAIBAOXIANG, "Bách b?o r??ng", szMsg, nEndTime, nAddTime);
end
...
function Baibaoxiang:ClearHelpTable()
	...
	Task.tbHelp:AddDNews(Task.tbHelp.NEWSKEYID.NEWS_BAIBAOXIANG, "Bách b?o r??ng", "", nEndTime, nAddTime);
end
...
function Baibaoxiang:OnChangeBack(nType, tbItem)
	...
	if nExCount <= 0 then
		Dialog:Say("Xin vui lòng ??t các m?c chính xác");
		return 0;
	end
	...
	if nType == 1 then
		...
		me.Msg(string.format("Nh?n ???c <color=yellow>%s<color> tinh l?c", nExCount * 225));
		
		me.ChangeCurGatherPoint(nExCount * 200); 	-- huoli		
		me.Msg(string.format("Nh?n ???c <color=yellow>%s<color> ho?t l?c", nExCount * 200));
		
	elseif nType == 2 then
		me.AddStackItem(18, 1, 205, 1, nil, nExCount);
		me.ChangeCurGatherPoint(nExCount * 100); 	-- huoli
		me.Msg(string.format("Nh?n ???c <color=yellow>%s<color> ho?t l?c", nExCount * 100));
	
	elseif nType == 3 then
		me.AddStackItem(18, 1, 205, 1, nil, nExCount * 200);
		me.ChangeCurGatherPoint(nExCount * 20000); 	-- huoli
		me.Msg(string.format("Nh?n ???c <color=yellow>%s<color> ho?t l?c", nExCount * 20000));
	end
end
