...
-- 为帮会和下属家族设置周目标（帮会ID，帮会的低级目标，帮会的中级目标，帮会的高级目标）
...
function Tong:SetCapital_GS1(nTongId, nDomainId, bConfirm)
	...
	if self:CanCostedBuildFund(nTongId, nSelfKinId, nSelfMemberId, nCost) ~= 1 then
		Dialog:Say("能使用建设资金额度不足！请让帮会的<color=yellow>首领<color>设置更高的周使用上限！");
		return 0;			
	end
	if bConfirm == 1 then
		return GCExcute{"Tong:SetCapital_GC", nTongId, nSelfKinId, nSelfMemberId, nDomainId};
	else
		local szMsg = string.format("您要将帮会的主城更换为<color=green>%s<color>。这是您的帮会<color=green>第%d次<color>更换主城，需要支付<color=green>%d<color>帮会建设资金。",
			szCapital, nChangeCount, nCost);
		Dialog:Say(szMsg, 
			{
				{"确定", self.SetCapital_GS1, self, nTongId, nDomainId, 1},
				{"我再想想"},
			});
	end
end
...
function Tong:SetCapital_GS2(nTongId, nDomainId, nCost, nChangeCount, nDataVer)
	...
	if szCapital then
		...
		KTong.Msg2Tong(nTongId, 
			string.format("帮会扣除建设资金<color=green>%d<color>，主城更改为<color=green>%s<color>。",
			nCost, szCapital));
	end
end
...
function Tong:PresidentConfirm_GS2(nTongId, tbResult, nDataVer, nKinDataVer)
	...
	if szName then
		KTong.Msg2Tong(nTongId, string.format("<color=green>%s<color>%s新一届的首领。", szName, szType))
	end
end
...
function Tong:PresidentCandidateConfirm_GS2(nTongId, nKinId, nMemberId, nDataVer, nKinDataVer)
	...
	local szName = KGCPlayer.GetPlayerName(pMember.GetPlayerId());
	if szName then
		KTong.Msg2Tong(nTongId, string.format("<color=green>%s<color>获得下一任首领候选资格，如果下周一仍然能维持股权第一的位置，将会成为新一任的首领！", szName))
	end
end
...
function Tong:OfficialMaintain_GS2(nTongId, nKinId, nMemberId, nPlayerId, 
	...
	if pPlayer then
		...
		pPlayer.Msg("Danh hiệu được duy trì thành công.");
	end
...
function Tong:EndGreatMemberVote_GS2(nTongId, nWeekGreatBonus, tbGreatMemberInfo)
	...
	if pTong then 
		...
		KTong.Msg2Tong(nTongId, "Kết thúc bầu chọn bang hội");
	...
	if not nChoose and nWeekBonus > 0 then
		...
		for nIdex = 1, #tbAwardMode do
			...
			if (nItemNum == nil or nItemLevel == nil) then 
				szSay = string.format("奖励方式%d:<color=green>%d绑定银两<color>", nIdex, nMoney);	
			elseif (nMoney == nil) then 
				szSay = string.format("奖励方式%d:<color=green>%d个%d级绑定玄晶<color>", nIdex, nItemNum, nItemLevel);
			else
				szSay = string.format("奖励方式%d:<color=green>%d个%d级绑定玄晶，%d绑定银两<color>", nIdex, nItemNum, nItemLevel, nMoney);
			end
			table.insert(tbOpt, { szSay, self.ReceiveGreatBonus, self, nIdex, 1 })
		end
		table.insert(tbOpt, {"结束对话"});
		szMsg = szMsg.."<color=yellow>恭喜你成功当选本周帮会的优秀成员。<color>\n您可以选择领取如下中的一种奖励：";
		Dialog:Say(szMsg, tbOpt);		
		return 0;
		
	elseif nWeekBonus <= 0 then
		Dialog:Say("本周的奖励基金为<color=yellow>0<color>");
		return 0;
	end
...
function Tong:ReceiveGreatBonus_GS2(nTongId, nSelfKinId, nSelfMemberId, nBonus, nMoney, nAwardMode)
	...
	if self:ReceiveAward(nMoney, nPlayerId, nAwardMode, "Tong") == 1 then
		if pPlayer then
  			pPlayer.Msg("领奖成功");
  			KTong.Msg2Tong(nTongId, "<color=green>"..pPlayer.szName.."<color>领取了优秀成员奖励。");
  		end
  		return 1;
  	end
  	if pPlayer then
...
function Tong:AdjustGreatBonusPercent(nPercent, bConfirm)
	...
	if nPercent and not bConfirm then
		...
		table.insert(tbOpt, { "确定", self.AdjustGreatBonusPercent, self, nPercent, 1 });
		table.insert(tbOpt, {"结束对话"});
		Dialog:Say(szMsg, tbOpt);
		return 0;	
	end
...
function Tong:AddGreatBonus_GS2(nTongId, nBonus, nPlayerId, nMoney)
	...
	if nPlayerId then
		...
		if cPlayer and nMoney and nMoney > 0 then
			cPlayer.Msg("您加了帮会奖励基金<color=yellow>"..nMoney.."<color>");
			return 0;
		end
	end
end
