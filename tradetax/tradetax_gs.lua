	...
	if nWelLevel ~= 0 and self.WELFARE_LEVEL[nWelLevel] then
		...
		if bConfirm then
			if pPlayer.nCashMoney + nWelfare > 2000000000 then 		-- 大于20亿
				pPlayer.Msg("Số tiền giao dịch quá giới hạn cho phép được mang theo, hãy kiểm tra lại.");
				return -2;
			end
...
function TradeTax:TradeMoney(pPlayer, nMoney)
	...
	local szMsg = "Số tiền tích lũy giao dịch trong tuần này là: ".. nCurMount + nMoney .. " lượng, thuế lần này là: ".. nTax ..string.format(" lượng, tỷ suất tìm phục lợi quan ở Lâm An Phủ để xem. Thông qua thỏi vàng Kỳ Trân Các, bạc nhận được khi mua %s không cần nộp thuế.",IVER_g_szCoinName);
	pPlayer.Msg(szMsg);
	if nTax > 0 then
		-- 记录交税情况
